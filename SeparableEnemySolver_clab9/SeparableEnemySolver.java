...
public class SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        ...
        HashMap<String, Integer> splited = new HashMap<>();
        HashMap<String, Boolean> checked = new HashMap<>();
        for(String invitee: invitees) {
            splited.put(invitee, -1);
            checked.put(invitee, false);
        }

        for (String invitee: splited.keySet())
        {
            if (splited.get(invitee) == -1) {
                boolean cansplit = mark(splited, checked, invitee, 0);
                if (!cansplit) {
                    return false;
                }
            }
    ...
    private boolean mark(HashMap<String, Integer> splited, HashMap<String, Boolean> checked, String node, Integer A){
        if (splited.get(node) == (A+1)%2) {return false;}
        if (!checked.get(node)) {
            checked.put(node, true);
            splited.put(node, A);
            for (String neighbor : g.neighbors(node)) {
                if (splited.get(neighbor)==-1) {
                    splited.put(neighbor, (A + 1) % 2);
                }
                else if (splited.get(neighbor)==A){
                    return false;
                }
            }
            for (String neighbor : g.neighbors(node)) {
                boolean cansplit = mark(splited, checked, neighbor, (A + 1) % 2);
                if (!cansplit) {
                    return false;
                }
            }
